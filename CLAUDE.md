...
| `chains-WC-<year>.parquet` | `build_chains_ci.R`, additive | Per-tournament shard of chains-WC (pannadata#109 — the WC monolith spans all 16 backfilled tournaments 1966-2026, 32MB/1.5M rows, crashed mobile browsers; shards measured ~1.2-4MB each and the monolith is still shipped unchanged for back-compat) |
...
```
pannadata scraper → GitHub Releases (opta-latest)
    ↓
panna::pb_download_opta() syncs from the release
    ↓
...
**TL;DR fix matrix** (symptom → fix; details below): stale/short `events_consolidated` → `rebuild-events.yml` (NOT `force_rescrape`); scrape returns `Found 0 total, 0 new` → missing `TOURNAMENT_DATE_EXCEPTIONS` entry (NOT "Opta gap"); cron daily-opta-scrape red on non-blog leagues → backlog backfill via `rebuild-events.yml` / daily heal pass; "corrupt" chains equity → metric-mismatch, not a join bug (don't diff `epv_credit`); a player showing at their OLD club on the blog despite a real transfer → check `daily-opta-scrape.yml`'s "Build current club squads" step logs for `GUARD FAILURES` / "Refusing to write", not just Opta feed lag.

- **`build_squads.py`'s ambiguous-transfer guard used to freeze the WHOLE `squads.parquet` file, not just the ambiguous player** (fixed 2026-09-11). When a player_id appeared in two leagues with no recognizable feeder/reserve pattern (a genuine in-flight transfer or data glitch), `main()` returned nonzero and the whole file was refused — masked by the daily workflow's `continue-on-error`, which just logs a warning and keeps the previous file. At 49-league scale during a transfer window this became a near-permanent failure mode: confirmed live, the file was frozen on its 2026-08-30 build for 12+ days (reported as "Barcola still shows at PSG"). Now drops just the ambiguous player(s) (they fall back to stale team data via `ratings.parquet`'s existing "uncovered league" contract) and writes everyone else — matching how the script already resolves a genuine structural (repeated) feeder/reserve pattern rather than blocking the whole file. If `squads.parquet`'s `build_id` looks stale, check the run log for `GUARD FAILURES` before assuming a scrape outage.
...
- **Backfilling events for a competition or onboarding a new one** — `rebuild_events.py` (per-comp backfill of stale/short `events_consolidated`, dispatched via `rebuild-events.yml`) and `discover_seasons.py` + `merge_discovered_seasons.py` (find and register a new competition's season IDs) are rare, manual, multi-step runbooks — see the `rebuild-opta-events` skill for the full sequence, exact commands/flags, and gotchas.
