...
**Opta** is the primary data source (15 leagues, ~280 columns per player per match, event-level data with x/y coordinates). Understat and FBref provide supplementary coverage.

## Data Sources

| Source | Coverage | xG Model | Storage | Key Strength |
|--------|----------|----------|---------|--------------|
| **Opta** | 15 leagues, 2013+ | SPADL + XGBoost | Parquet per season, Parquet consolidated | ~280 columns, event coordinates, progressive carries |
...
Per-player-per-match statistics. **~280 columns** covering every aspect of match performance. Key columns listed below — the full set is available via `load_opta_stats()`.
...
| `goalmouth_y` | num | Goal-line crossing point, horizontal (Opta q102; 0-100 width scale, posts ~45.2/54.8). `NA` if absent. Reliably present 2021-22+; backfilled for history via `scripts/opta/backfill_goalmouth.py`. Feeds the xGOT (post-shot xG) model. For Miss shots (`type_id==13`) specifically, sourced from q231 instead of q102 (q102 scatters far outside the goal frame for genuine misses; q231 validated via Post-hit shots landing at the known post positions — panna#175/inthegame-blog#489), falling back to q102 when q231 is absent (~13-30% of misses). Historical rows corrected via `scripts/opta/backfill_miss_shot_y.py`. |
...
Pre-computed xG, xA, and xPass metrics per player per league-season. Generated by the SPADL + XGBoost pipeline (`03_calculate_player_xmetrics.R`) from match_events data. Aggregated across all matches in a season.
...
| `penalty_goals` | int | Penalty goals |
...
| `xpass_avg` | num | Average xPass per pass |
| `goals_minus_xg` | num | Goals minus xG (overperformance) |
| `xg_per90` | num | xG per 90 minutes |
| `xa_per90` | num | xA per 90 minutes |
| `xpass_overperformance_per90` | num | xPass overperformance per 90 |
...
| Column | Type | Description |
|--------|------|-------------|
| `id` | int | Understat internal row ID |
...
| `yellow_card` | int | Yellow cards |
| `red_card` | int | Red cards |
| `x_g` | num | Expected goals |
| `x_a` | num | Expected assists |
| `x_g_chain` | num | xG chain (involvement in possessions ending in shot) |
| `x_g_buildup` | num | xG buildup (involvement minus final action) |
| `h_a` | chr | Home or away |
| `roster_in` | int | Minute subbed on (0 if starter) |
| `roster_out` | int | Minute subbed off (0 if full match) |
| `league_code` | chr | League code |
| `season_input` | chr | Season input string |
...
| `date` | chr | Match date |
| `player_assisted` | chr | Assist provider |
| `last_action` | chr | Last action before shot |
| `league_code` | chr | League code |
| `season_input` | chr | Season input string |
...
| Column | Type | Description |
|--------|------|-------------|
| `understat_id` | int | Understat match ID |
| `match_url` | chr | Understat match URL |
